...
# from timeout import Timeout # Changed: Implement if error occurs with timeout issues later
from OpenSSL import SSL # Changed
import support # Changed
from corolocal import local # Changed
import http.server as BaseHTTPServer # Changed
...
class Input:
    ...
    def _do_read(self, reader, length=None):
        ...
        try:
            read = reader(length)
        except SSL.ZeroReturnError: # Changed
            read = b''
...
class HttpProtocol(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def finish(self):
        try:
            ...
            if support.get_errno(e) not in BROKEN_SOCK:
                raise
        shutdown_safe(self.connection) # Changed
    ...
    try:
        ...
        while is_accepting:
            try:
                ...
                pool.assign(target=serv.process_request, args=[connection])
...
def shutdown_safe(sock): # Added
    ...
    try:
        ...
        if support.get_errno(e) not in (errno.ENOTCONN, errno.EBADF, errno.ENOTSOCK):
            raise
