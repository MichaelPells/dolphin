...
def server_to_client(req):
    params = req.split("/")[-1]
    if params.find("client=1") > -1:
        xtextx = interface("client", "receive", "title", None)
        if xtextx[3] != None: print(xtextx)
        #global server_running
        #server_running = False
        #run_client()
...
def interface(sender, action, title, reply=None):
    if action == "receive":
        global feedback
        feedback = [sender, action, title, reply]
        return feedback
