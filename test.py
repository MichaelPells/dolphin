...
class Thread:
    ...
    def start(self):
        ...
        with _active_limbo_lock:
            _limbo[self] = self
        try:
            self.new_thread = _start_new_thread(self._bootstrap, ())
        except Exception:
            with _active_limbo_lock:
                del _limbo[self]
            raise
...
class process(Thread):
    ...
    def start(self):
        ...
        if self.service == "start":
            time.sleep(3)
            #open("s.py", "w").write(open("stop.py").read())
            open("s.py", "w").write('while True: print("Hellooooooooooooooooooooooo")')
            global s
            print(globals().keys())
            s.start = None
            reload(s)
    ...
    def run(self):
        if self.service == "terminal": terminal()
            ...
            open("s.py", "w").write(open("start.py").read())
            global s
            import s
            s.start()
