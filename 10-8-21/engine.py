...
def output(message="", style="new"):
    if style == "new": open("engine_outputs.txt", "w").write(message)
    if style == "update": open("engine_outputs.txt", "a").write(message)
...
def server_log(message=None):
    if message != None: open("data/server_log.txt", "a").write(message)
        ...
        output(open("data/server_log.txt").read())
...
def server(**params):
    ...
    if len(arguments) > 0: header = arguments[0]
        ...
        output("Incomplete parameters given!")
        return
    arguments.pop(0)
...
def browse(**params):
    ...
    try: source = requests.get(url, allow_redirects=True)
    except Exception as error: output(str(error))
    else:
        if source.status_code == 200:
            ...
            update_(["client", "/".join(source.url.split("/")[3:]), t, source.status_code])
            output(name+" copied successfully!")
            
        else: output("File could not be copied: "+source.reason.replace("File",file))
...
def history(**params):
    ...
    if option == "client":
        ...
        List = log["server"]
        output(json.dumps(List))
    else: output("Unknown Response: Please select an available option.")
