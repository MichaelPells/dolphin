...
def run_client(lastUsedCoord=None):
    ...
    if len(linkList) > 0:
        for ln in range(0, len(linkList)):
            visible.append("  "+str(ln+1)+"  -  "+linkList[ln])
    ...
    for file in files:
        ...
        except Exception as error: interface("output", "send", [str(error)])
        else:
...
def feedback(title, process, status):
    ...
    if title == "process":
        if status == True:
            if process != "menu":
                if "menu" in log["feedback"]["processes"]: log["feedback"]["processes"].remove("menu")
                if process not in log["feedback"]["processes"]:
                    log["feedback"]["processes"].append(process)
                    released = release_interface_log(log)
            else:
    ...
    if not released: release_interface_log()    
...
def interface(sender, action, title, reply=None):
    ...
    if action == "receive":
        ...
        release_interface_log(log, True)
...
class process(threading.Thread):
   ...
   def run(self):
       if self.name == "console": import console
