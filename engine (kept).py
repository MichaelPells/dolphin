...
def output(*message):
    if len(message) > 0:
        message = {"service": Input["service"], "level": Input["level"], "no": Input["no"], "code": message[0], "text": message[1], "timestamp": time.time()}
...
def stop(**params):
    ...
    if subsystem in InputFlowBase:
        ...
        try:
            os.kill(subsystem_PID, signal.SIGTERM)
            del InputFlowBase[subsystem]
            output("CODE", subsystem+" stopped!")
        except Exception: output("CODE", "Stop failed: "+subsystem+" could not be stopped.")
        
    else: output("CODE", "Stop failed: "+subsystem+" does not exist.")


def roll(service, handover={"flow": []}):
    if service == "start": component = Process(target=start, kwargs={"handover": handover, "modules": modules})
    elif service == "stop": component = Process(target=stop, kwargs={"handover": handover, "InputFlowBase": InputFlowBase, "modules": modules})
    elif service == "server": component = Process(target=server, kwargs={"handover": handover, "modules": modules})
    elif service == "browse": component = Process(target=browse, kwargs={"handover": handover, "modules": modules})
    elif service == "history": component = Process(target=history, kwargs={"handover": handover, "modules": modules})
...
def main():
    ...
    global raw_modules
    raw_modules = [copy, datetime, email.utils, html, http.client, io, mimetypes, posixpath, select, shutil, socket, socketserver, sys, time, urllib.parse, contextlib, partial, HTTPStatus, argparse, json, requests, signal, threading, Path, Process]

    for module in raw_modules:
        globals()[module.__name__.replace(".","")] = module

    global modules
    modules = [copy, datetime, emailutils, html, httpclient, io, mimetypes, posixpath, select, shutil, socket, socketserver, sys, time, urllibparse, contextlib, partial, HTTPStatus, argparse, json, requests, signal]
    ...
    global Input
    Input = {"service": None, "level": 0, "no": "0"}
    output(__name__, str(PID))
    open("running_engines.pid", "a").write(str(PID)+"\n")
        ...
        while True:
            if NewInput:
                ...
                if service in allowed_services:
                    ...
                    Input["level"] += 1
                    open("hello.txt", "w").write(str(time.time()))
                    roll(service, Input)
                NewInput = False
