...
def server_log(message=None):
    if message != None: open("data/server_log.txt", "a").write(message)
        ...
        output(Input, "CODE", open("data/server_log.txt").read())
...
def browse(**params):
    ...
    while trial <= 4:
        ...
        if attempt[0] != None:
            ...
            trial += 1
            if trial == 4:
                output(Input, "CODE", "Server not found!")
                return
    ...
    try: source = requests.get(url, allow_redirects=True)
    except Exception as error: output(Input, "CODE", str(error))
    else:
        if source.status_code == 200:
            ...
            output(Input, "CODE", name+" copied successfully!")
            
        else: output(Input, "CODE", "File could not be copied: "+source.reason.replace("File",file))
...
def history(**params):
    ...
    if option == "client":
        List = log["client"]
        output(Input, "CODE", json.dumps(List))
    elif option == "server":
        List = log["server"]
        output(Input, "CODE", json.dumps(List))
    else: output(Input, "CODE", "Unknown Response: Please select an available option.")
